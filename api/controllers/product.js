...
exports.create = async (req, res) => {
    try {
        const requiredFields = ['name', 'code', 'price', 'unit', 'selling_price'];
        ...
        const existUser = await products.findOne({
            where: {
                deleted: { [Op.eq]: 0 },
        ...
        if (existUser) {
            return res.status(400).send({ message: "Produk Telah Terdaftar!" })
        }
        const payload = {
            ...req.body,
